...
**Upwork messages:**
- No markdown formatting (no bold, no headers, no `**text**`). Upwork strips markdown. Plain text only.
- No bullet points (`-` or `*`). Use plain text with line breaks for sub-items.
- Numbered sections OK for structure (`1)`, `2)`, `3)`). Use them for multi-topic messages (recaps, status updates). Label each section with a short descriptor (e.g., `1) Hot leads fix:`).
- Put email addresses, file names, and links on their own line for visual emphasis. Don't bury them inline.
...
When the client spotted an issue, raised a good question, or made a useful suggestion, call it out by name. Don't just state the fix -- credit the person who flagged it.

**Good:** "Good spot, Jess! The Birmingham template had..."
**Bad:** "The Birmingham template has been corrected."

### Offer Agency

When presenting a new workflow or process (e.g., "send corrections to this email"), also mention the client can do it themselves if applicable. Give them the choice rather than prescribing a single path.

**Good:** "...forward it to client.meji-media@unpauseai.com with a short note. Alternatively you can edit it yourselves, of course. Whichever works best for you."
**Bad:** "...forward it to client.meji-media@unpauseai.com with a short note of what needs changing."

### Don't Over-Section

Combine related items naturally. Don't create a separate bold header for every piece of info. Meeting recordings, links, and supporting references belong inline where they're contextually relevant, not as standalone sections.

### Conversational Framing for Post-Meeting

After a call or meeting, start with context ("So as promised, here's the post-meeting summary") not pleasantries ("Great meeting you today"). The greeting already happened in the meeting. Jump to value.
