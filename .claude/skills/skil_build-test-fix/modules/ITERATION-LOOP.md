...
## Hard Gate: Visible Iteration Counter

Before EVERY fix attempt, state this in the response:

> **Iteration {N}/3** -- Category: {error_category} -- Previous: [{fix_1}, {fix_2}, ...]

Stop conditions (mandatory):
1. **N > 3:** STOP. Escalate immediately using Escalation Format below.
2. **Same category + same fix approach as previous iteration:** STOP. Going in circles.
3. **Novel error (not in FIX-PATTERNS.md):** 1 attempt max. If that fails, escalate.

Exceeding 3 iterations is a friction event (`iteration-limit-breach`).

**Anti-pattern:** "Let me try one more thing" after iteration 3 has historically wasted 15+ min per attempt with near-zero success rate.
