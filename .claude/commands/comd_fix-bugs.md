---
description: Fix failing tests for an automation using the agnt_bug-fixer agent
argument-hint: <project-name> <automation-id>
---

# Fix Bugs

Automatically fix failing tests for an automation using the agnt_bug-fixer agent.
...
Launch the agnt_bug-fixer agent with:
- **Client**: {client}
...
Use the Task tool:
```
Launch agent: agnt_bug-fixer

Prompt:
Fix failing tests for automation {automation_id} in client {client}.

Process:
1. Resolve project directory: check `workspace/clients/{project}/` first, then `workspace/projects/{project}/`. Navigate to the automations subfolder.
...
After fixing:
- Update spec frontmatter `last_changes`
- Generate fix report summary
```

## Step 3: Review Fix Report

The agnt_bug-fixer agent will output:
- Files modified
- Fixes applied
...
| Situation | Action |
|-----------|--------|
| Agent not found | Ensure agnt_bug-fixer agent exists in `.claude/agents/` |
...
- This command uses the agnt_bug-fixer agent for automated fixing
- The agnt_bug-fixer follows minimal changes principle
- Always reviews test output before applying fixes
- Re-runs tests after fixing to verify
- Updates spec frontmatter to track changes
