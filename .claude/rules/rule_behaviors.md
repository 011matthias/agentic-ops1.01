...
**When the decision point is genuine, it still has to be a decision.** The
2026-08-23 and 2026-08-24 blocks both landed on work that really was invasive
(writes into a client's live financial records), so stopping was correct and
the phrasing was not. Two moves both repeats missed: split the action and take
its read-only half first (reading the records to say what they actually are is
yours to do, and often dissolves the question), then put the remainder as a
decision with a recommendation and its consequence, via `AskUserQuestion`. "Say
the word and I'll ..." is an open offer, not a decision point; it hands back
the judgment the user was relying on you to supply.
...
**Fix-bites-the-caller (B2 sub-clause).** A green suite proves the helper works; it does not prove the helper is WIRED. So a fix's tests must include at least one assertion that runs THROUGH the caller the fix changed, not only through the helper it added, and the acceptance criterion is not "the suite is green" but **regress the real source and watch it go red**: disable the fix at its wiring point, run the suite, confirm a caller-level test fails, restore. Run it, don't trust it — `uv run tools/regress_check.py --test "{cmd}" --file {src} --replace "{wired call}" --with "{disabled}"` does the whole loop (green → red → restored → green) and exits 1 with `TEST DOES NOT BITE` when the suite survives the mutation. The test that went red is the answer to B2.3's "name the specific test performed"; a fix whose suite has never been seen to fail has not been verified, it has been asserted. Source: 2026-08-24, the first draft of `test_card_from_receipt.py` had 9 of 10 tests calling `_payment_mode` / `_card_last4` directly, so unwiring the ingest's call to the merge left all ten green; caught only by hand-regressing the source. Recurrence of the 2026-08-22 verification-theater row, whose fix was `documented` and did not hold.
...
**Consumer-drive sub-clause (deploy verification gate).** A deploy is verified when the thing that CONSUMES the changed payload has rendered it. `/healthz`, API reads, `curl` and WebFetch all exercise the server, and for an SPA route WebFetch returns the shell rather than the rendered state, so passing them proves the half that was not broken. On 2026-08-24 every server check passed while the SPA rendered the new `pooled` status as "Arriving" with a blank Month, and six real receipts misreported until a human found it (same class as the 2026-08-22 blank page three days earlier). Drive the consumer instead: Playwright MCP `browser_navigate` + `browser_snapshot` on the route that displays the changed field, asserting the NEW value renders and that no fallback took its place ("Arriving", "--", "Unknown", a blank cell). Structural enforcement (built 2026-08-24): `.claude/hooks/deploy-consumer-gate.py` opens a marker on every deploy and blocks a Stop whose text claims verification while it is still open. What closes the marker depends on the surface: an app deploy (fly / railway / wrangler) puts a client-side renderer between the API and the truth, so only a browser drive closes it, and a `curl` / WebFetch against it re-states the gap instead; a server-rendered deploy (vercel) is closed by the no-slash URL fetch this rule already mandates. Keeping the two apart is deliberate: a guard that fires on every platform ship would be approved reflexively and would then protect nothing. The Stop arm honors `stop_hook_active`, so it costs one turn and cannot wedge a session. Tests: `tools/tests/test_deploy_consumer_gate.py`. **Tightened 2026-09-15: driving is not observing.** Until then ANY browser call closed the marker, `agent-browser open <url>` and `browser_navigate` included, which prove a page was requested and nothing about what it rendered; the gate printed "[CONSUMER DRIVEN] ... closed" for a backgrounded command that had asserted nothing and then timed out. A gate closable by something that proves nothing protects less than it appears to, because its own advisory reads back as evidence. Closing now needs a command that READS PAGE STATE BACK (`snapshot`, `get`, `eval`, `screenshot`, `find`, `wait --text`; `browser_snapshot`, `browser_evaluate`, `browser_take_screenshot`), run in the FOREGROUND and not reporting failure. Navigation and interaction stay silent rather than re-advising: they are a drive in progress, not a substitute for one. Fail-open on an unreadable tool response, because a gate that refuses to close on a drive that did happen becomes noise, and noise is approved reflexively.

**Cold-drive sub-clause (deploy verification gate).** Drive it from the seat the RECIPIENT sits in, not the one you already hold. A drive that starts from a session, cookie, token or local state you minted for yourself skips the entry path the third party actually takes, which is where the break hides; the hook cannot tell the two apart, so it closes on either. So for anything a third party will open (a shared link, an invite, a gated page, a handoff URL), the FIRST drive is cold: no session, from their entry point, all the way through whatever gate stands in front of it, and the assertion is that they land on the thing, not that the thing renders once you are past the gate. Where the entry path runs through mail, drill the delivery leg self-addressed (never to the recipient) and confirm arrival at the source. Source: 2026-09-07, a Lead Desk review packet was declared live after a browser drive with a session cookie minted from the app secret; the drive proved the authenticated render while sign-in still dropped the recipient on the board with no route to the packet, and the gap surfaced only because the owner pasted a screenshot of the login page and asked whether the link would work. Recurrence of the 2026-08-24 consumer-drive row, whose structural fix holds for "was a consumer driven" and not for "was it driven cold".

**Instrument-validity sub-clause (B2).** Driving the consumer answers "did I look at the right thing"; it does not answer "can what I looked THROUGH see the thing at all". A probe that is structurally blind returns a confident negative, and a negative is the dangerous direction: it reads as "the change did not land" and invites a retry of work that already succeeded. Three shapes, all live in Aug-Sep 2026: an endpoint that is empty **by design** for the caller (Vinted `/api/v2/users/{uid}/items` is always empty for one's own closet, so three successful publishes read as failures and the retry loop put three duplicate listings on a real account, 2026-09-08); a filter the API **silently ignores** rather than rejecting (Instantly dropped an unknown `campaign_id` and answered workspace-wide, so per-campaign counts were workspace counts, 2026-08-27, recurring 2026-09-06 as a 267/258 double-count); and an artifact that was **true earlier** (a 13:04 bundle audit still asserting "unapplied" that evening while the owner's own report showed the page live, 2026-09-08).

So before a probe's output grounds any claim, prove the instrument on a state you already know:

1. **Differential-probe.** Run it against a case whose answer you know independently, and confirm the output CHANGES with the state. A probe that returns the same thing for "present" and "absent" has told you nothing. For a filter or query param, verify it narrows: an unfiltered call and a filtered call returning the identical count means the filter was dropped, not that everything matched.
2. **Prefer the surface the actor writes to.** Read state back through the path that the state actually lands on (the wardrobe/item endpoint the publish creates, the record the write touched), never through a sibling that merely sounds right.
3. **Freshness beats recall.** An audit, bundle, ledger, or snapshot is evidence only about the moment it was taken. If anything could have changed since — a deploy, a publish, an owner action — re-run it before asserting current state. A stale artifact never outranks a live report from the person looking at the thing.

A confident negative from an unvalidated probe is `verification-theater` even when every command exited 0, and it is the one shape of it that generates real-world damage rather than just a wrong sentence.

**Whole-file-write floor (B2).** `Write` is whole-file replacement: everything absent from the new content is deleted, and nothing in the harness reports the delta, so the loss is invisible at the moment it happens and surfaces later, if at all. Read-before-overwrite does not catch it; it passed in both 2026-08 incidents. A same-named Write clobbered a 598-line test module because the read predated a compaction (caught only when the suite count dropped 1168 to 1153), and a loop-brief rewrite deleted 504 lines including the owner's live open queue (caught only from the commit stat). Prefer `Edit` for anything partial: a targeted Edit cannot delete what it does not name. On a file that accumulates state, the small diff is the correct instinct. Structural enforcement (built 2026-08-24): `.claude/hooks/write-shrink-gate.py` asks on a material shrink and lists the headings and defs about to disappear, so they are read before they are gone. Tests: `tools/tests/test_write_shrink_gate.py`.
