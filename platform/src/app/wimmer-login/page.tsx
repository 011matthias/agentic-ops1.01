// Server-rendered login page for the client doc sites (Wärme Wimmer + Meji
// Media; site selected via ?site=). Visually identical to the in-page overlay
// it replaces (same logo, card, gradient OK button, error line). Form posts to
// /api/wimmer-unlock which validates and sets the per-site HMAC cookie.

import { DOC_SITES, parseSiteKey, safeFromUrl } from "@/lib/wimmer-auth"

export const metadata = {
  title: "Documentation Access | UnpauseAI",
  robots: { index: false, follow: false },
}

type Props = {
  searchParams: Promise<{ from?: string; err?: string; site?: string }>
}

export default async function WimmerLoginPage({ searchParams }: Props) {
  const { from, err, site } = await searchParams
  const siteKey = parseSiteKey(site)
  const target = safeFromUrl(from, siteKey)
