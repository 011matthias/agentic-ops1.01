...
- **Commands** (29) — Your interface. Type `/command` to invoke.
- **Skills** (34) — Domain expertise (first-party `skil_` dirs; ~11 are pack-consolidation stubs hosting modules for make-pack/n8n-pack), plus vendored plugin skills. Auto-load by context (e.g., Make.com work loads make-pack).
...
- **Rules** (15) — Always-on constraints in `.claude/rules/` (behaviors, session start, session pressure, plus the hard-constraint standards). Orchestrator detection deferred to build-time.
