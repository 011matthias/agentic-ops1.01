...
- Status + backlog: item 204 D2/D3 paragraph, new item 214, status top paragraph (docs PR).
...
## Next Steps
1. Build item 214 (continuation prompt below).
2. Criss: September's 9693 rows after Sep 15 and the 2838 family for September arrive with her next uploads; until item 214 ships, only a single-month file is safe to attach.

## Files to Read First
- `workspace/clients/brisken/status/p1-improvement-backlog.md` item 214 and the D2 / D3 paragraph at the end of item 204
...
# Brisken p1 expense-recon: build item 214, the statement attach keeps only the month's own charges (owner decision 2026-09-25, D2 going forward)
...
## The job, in order
1. Read item 214 in `workspace/clients/brisken/status/p1-improvement-backlog.md` and the D2 / D3 paragraph closing item 204.
...
## How to work
- Branch `client/brisken/p1-item214-attach-month-guard`, worktree `C:\Users\neuma_p1qrsic\Repo\agentic-ops1-214` off origin/main, follow `workspace/clients/brisken/automations/expense-reconciliation/docs/PARALLEL-ROUND-PROTOCOL.md`.
...
- Record the outcome as a dated paragraph in item 214 and one at the top of `status/p1-expense-reconciliation.md`, through a `docs/...` PR.
- Checkpoint: `/comd_checkpoint --mini`, topic `Expense-Recon Item 214 Attach Month Guard`, docs branch cut fresh off origin/main in its own worktree, `finalize --root` that worktree.
