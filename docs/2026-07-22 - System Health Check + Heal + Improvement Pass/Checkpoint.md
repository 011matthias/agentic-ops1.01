...
1. **Apply the 4 reconcile upgrades?** `uv run tools/brisken-outreach-reconcile.py --write` (backup + exact-diff verify built in), or leave for Dirk. The 1 hold row is his either way.
2. **Zero-guard hard-die?** #391 shipped the loud warning; if you want `start` to refuse any manifest with zero guards outright, it is one line + fixture updates.
3. **Remaining optimize verify items** above — none block runs; #6 (timeout attribution) and #23 (correlated guards doctrine) are the two worth a future round. S1 stays blocked on re-validating the label fixture.
