...
2. **Residual validator findings** above: next `/comd_system-dev` round candidate (A2 and C4 are the highest-value, both FP-sensitive so they need care, which is why they were not rushed into #380).
3. **Freshness bootstrap fallback** (small wire-hooks build).
