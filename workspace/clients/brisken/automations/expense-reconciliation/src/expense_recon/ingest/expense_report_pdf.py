...
travel, 12 rows), ER-00139 (USD single-line flight). Row shapes
observed and covered:

* Row start: "<n>. MM/DD/YYYY" (numbered), "MM/DD/YY [1] [2]"
  (per-diem, trailing policy-violation footnotes), or a bare
  "MM/DD/YY" (continuation row after a page break, no number).
* Metadata lines: "Ref# :", "Payment Mode :", "Paid Through :",
  "Location :", "Attendees :", "Customer :", "Project :",
  "Per diem Name :", "Day(s) :", "Tax Name :" -- label-prefixed, any
  subset, any order.
...
_META = re.compile(
    r"^(Ref#|Payment Mode|Paid Through|Location|Attendees|Customer|"
    r"Project|Per diem Name|Day\(s\)|Tax Name)\s*:\s*(.*)$"
)
_FX_RATE = re.compile(r"^1\s+([A-Z]{3})\s*=\s*([\d.]+)\s+([A-Z]{3})$")
_NON_REIMB = re.compile(r"^Non Reimbursable$")
_INCLUDES_TAX = re.compile(r"^Includes Tax\s+[€$£][\d.,]+$")
...
def _resolve_amounts(classified: dict) -> dict:
    ...
    sym1, raw1 = first_monies[0]
    original_amount = _parse_money(sym1, raw1)
    original_currency = _SYMBOL_CCY.get(sym1, sym1)
    base_amount = None
    exchange_rate = fx[1] if fx else None

    if len(first_monies) >= 2:
        # Dual-column same-currency line: "Uber $60.85 $60.85"
        sym2, raw2 = first_monies[1]
        base_amount = _parse_money(sym2, raw2)
...
def _parse_money_for_ccy(ccy: str, raw: str) -> Decimal:
    raw = raw.strip()
    if ccy == "USD":
        return Decimal(raw.replace(",", ""))
    # EU format (EUR and others in these reports): period thousands (if
    # any), comma decimal.
    if "," in raw:
...
def _parse_money(symbol: str, raw: str) -> Decimal:
    return _parse_money_for_ccy(_SYMBOL_CCY.get(symbol, symbol), raw)
...
def _validate_currency_totals(
    ...
    for ccy, raw in zip(ccy_line, amount_line):
        try:
            printed = _parse_money_for_ccy(ccy, raw)
