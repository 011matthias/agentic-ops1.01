...
|---|---|---|
| `lovable-feedback-0917b-prompt.md` (notes #52, #53, #62, #63) | `category_confirmable`, `/confirm-category`, `expx.category.keep`, `expx.cardFix.change` in `chunk-expenses._batchId`; `receipt_image_available` + `expx.setaside.view` beside `SetAsideStrip`; `has_statement` + `rcpt.rematch.done` in `chunk-receipts`; the old `expx.review.reason.vendor_guess` text ("guessed from the file name") ABSENT and "up to 80 files" ABSENT from `rcpt.help` | Written 2026-09-17, not pasted. Backend live since Fly v150 (PR #988) |
