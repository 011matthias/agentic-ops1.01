...
| `lovable-settled-outside-prompt.md` (2026-09-15: a receipt paid by bank transfer, cash or PayPal can be retired from the pool without leaving the month) | some chunk reads `n_settled_outside`, `settled_outside` and `suggested_settled_outside`, and POSTs `/settled-outside`; the i18n chunk carries `wb.settledOutside.action`. Until then July's Redis, Konsultancy and 360Crossmedia invoices stay in the pool with no control to retire them | Backend live (item 62); the SPA half is this prompt |
| `lovable-receipt-coverage-prompt.md` (2026-09-15, item 68: how much of the month the report actually carries) | some chunk reads `n_receipts_in_report`; the i18n chunk carries `grid.receipt.coverage`. Until then nothing on screen says how many of the month's receipts are in the document | Backend live (item 68); the SPA half is this prompt. Paste item 67's `lovable-render-failed-prompt.md` FIRST: this one is its positive companion and deliberately adds no second per-row chip. The field is ABSENT until a report has been built, so absent must render as today, not as false |
...
| `lovable-cost-centers-prompt.md` (2026-09-15, item 47 steps 1-5 backend; §1 + §2 ship FIRST) | settings chunk reads AND writes `cost_centers` and the cards save payload carries `default_cost_center`; grid chunk reads `cost_center_source_label` and `cost_center_options`; a chunk fetches `/api/cost-centers/totals` | Backend on `client/brisken/p1-cost-centers` (NOT yet merged or deployed). Whole-map-replace gate: the owner must not type a cost center or card default until the published bundle proves §1 and §2 round-trip both maps |
| `lovable-rejected-pairing-prompt.md` (2026-09-15: a rejected receipt renders muted with an undo instead of being offered again) | some chunk reads `rejected` on a candidate and `n_rejected_pairings`; the i18n chunk carries `wb.cand.rejected` + `wb.cand.rejectedUndo`. Until then a rejected candidate is re-listed as if it were still an option | Backend live (item 16); the SPA half is this prompt. The undo it calls is the existing decision route with `status: pending`, so nothing else gates it |
| `lovable-amounts-unreadable-prompt.md` (2026-09-15: the count beside the month total) | some chunk reads `n_amounts_unreadable`; the i18n chunk carries `wb.total.amountsUnreadable.one` | Backend live (item 65); the SPA half is this prompt. The render is nothing-at-0 and the count is 0 on both live months, so applying it changes nothing visible today — that is the design, not a failed paste. The PDF half needs no SPA change |
| `lovable-adjacent-month-prompt.md` (2026-09-15: a receipt borrowed from the month next door is named on the row, on the candidate, and in a tile) | some chunk reads `from_batch` and `n_adjacent_borrowed`; the i18n chunk carries `wb.charge.fromMonth`. Until then a charge settled from a neighbouring month renders with no badge and an offered borrowed candidate looks like this month's own | Backend live (item 61); the SPA half is this prompt. No live case today: both live months already sit either side of a batch that lends them nothing in period, so the renderer has nothing to exercise until a September statement is attached |
| `lovable-render-failed-prompt.md` (2026-09-15: an expense whose receipt produced no page in the report says so on the row) | some chunk reads `receipt_render` and `n_receipts_unrenderable`; the i18n chunk carries `expense.notInReport`. Until then the only place a blocked receipt is named is a caption inside the PDF | Backend live (item 67); the SPA half is this prompt. No live case exercises the chip today: both months report 0 unrenderable, which is the correct state and not something to manufacture in Criss's data, so the renderer is bundle-audited like the blocked readiness bar |
| `lovable-statement-record-prompt.md` (2026-09-15, item 64: each statement row says which currency and which columns it was read with) | the statements-table chunk reads `card_currency` and `column_map`; the i18n chunk carries `stm.col.readAs` and `stm.readAs.unrecorded` | Backend live (item 64). Every upload in production predates the recording, so the applied render is the muted "Not recorded" until a month takes its next statement or is re-read |
| `lovable-workbench-filter-sort-prompt.md` (2026-09-15: amount sort parses the group separator; card / company / has-candidates filters; per-option counts; filter+sort state in the URL) | some chunk strips `[^0-9.-]` (or equivalent) before comparing `amount`; a chunk reads `coverage[].label` for a filter option; the run route writes `sort=` / `card=` to the query string. Until then "Amount, high to low" puts August's 2,484.00 and 1,574.24 at the bottom of the unmatched group | No backend gate: every field already ships. Browser-drive to verify, not bundle grep alone; the amount half is visible in the row order |
| `lovable-failure-probe-prompt.md` (2026-09-15, item 50: report a rejected request so the next "Failed to fetch" is provable) | some chunk POSTs to `/api/client-errors` and the body carries `seconds_ago` -- grep for BOTH, since a report without the elapsed time cannot answer the machine question and is the half most likely to be dropped | Backend on `client/brisken/p1-item-50-probe` (NOT yet merged or deployed). Silent by design: no screen, no toast, so a bundle grep is the only way to verify it -- a browser drive shows nothing either way |
| `lovable-view-receipt-prompt.md` (2026-09-15, item 52: the **View receipt** button opens the receipt) | some chunk renders the image blob by its own `type` (an `<iframe>` for `application/pdf`, an `<img>` for `image/*`) and the i18n chunk carries `receipt.view.failed`. Until then the button is enabled, has a handler, and does nothing | No backend gate: the image route serves all 31 August and 51 July receipts 200 today, and `receipt_image_available` is true on every one. **Browser-drive to verify, never bundle grep alone** -- the defect IS a click with no visible effect, so "no console error" is not a pass; the pass is a receipt on screen, on a statement month AND an open one |
