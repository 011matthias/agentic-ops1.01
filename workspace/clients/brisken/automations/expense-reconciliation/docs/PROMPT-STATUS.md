...
| `lovable-brisken-domain-prompt.md` half 2 (API base URL to `api.expenses.brisken.com`) | some chunk contains `api.expenses.brisken.com` AND no chunk contains `brisken-expense-recon.fly.dev` (one hit today) -- grep BOTH, since the old host keeps answering and a half-applied change reads as a mix | **Nothing gates it any more.** Half 1 is done: the SPA serves at `expenses.brisken.com`. `api.expenses.brisken.com/healthz` answers `ok` from the same machine as the fly.dev host (checked 2026-09-15). Paste it AFTER the other two, so the host replace sweeps up the receipt-image URLs the view-receipt prompt adds |
