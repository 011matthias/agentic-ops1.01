...
| `lovable-receipt-coverage-prompt.md` (2026-09-15, item 68: the Receipt column's third state, "No page in report") | some chunk reads `receipt_in_report` and `n_receipts_in_report`; the i18n chunk carries `grid.receipt.noPage`. Until then an unrenderable receipt still reads "attached" on the grid | Backend live (item 68); the SPA half is this prompt. Note the field is ABSENT until a report has been built, so the renderer must treat absent as today's two states, not as false |
