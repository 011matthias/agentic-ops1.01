...
def _create_batch(client, name="a.jpg", data=JPG, legal_entity="Corporate Services"):
    # 2026-09-08 split: empty company-month create + receipt add. Memory /
    # registry consult runs on the add path now, which is what the loop
...
def test_commit_then_next_batch_autofills_then_forget_stops(client, monkeypatch):
    ...
    # SPELLING resolves via the REGISTRY (naming is its job) and so does the
    # CATEGORY: one correction seeded both stores, and since note item M1
    # (2026-09-18) a merchant with a registry default reads that default,
    # while the per-entity learned row decides the ACCOUNT (the 2026-08-07
    # owner call, kept for the account: one merchant legitimately posts to
    # different accounts per legal entity). The divergence case itself is
    # pinned in test_merchant_registry.py.
