...
8.4 (done) → 8.5 column-add, all follow the run-log pattern. 8.1 was
...
precondition. Remaining: 8.5 export column-add (add the receipt-URL +
report-reference columns to `zoho_export.py`, wiring `resolve_receipt_urls`
for the URL and `ReportStore.report_for` for the reference), and the CLI
`store:` / `hosting:` opt-in that persists statements (8.2) + reports
(8.3) and hosts receipts (8.4) on a real run.
...
- [x] FX cases produce ≤ 2x the actual cross-currency-receipt count
      (instead of the original O(N×M) cross-product). **MET 2026-06-11.**
      3.7 (date + implied-rate-band emission gate) cut Needs-Review
      pair-rows 19-21x (10,124→545 / 6,624→337 / 3,273→153). 3.8
      (bipartite assignment) then collapsed each foreign receipt to its
      single best transaction: 43 / 35 / 18 FX pairs for the three
      months — ~1x the foreign-receipt count (44 / 35 / 19), inside the
      ≤2x target. 3.9's vendor signal chooses WHICH transaction wins
      each receipt (verified on real data: "Mega Center"→"MEGA CENTE
      CONSTR" sim 0.86, "Barreiros"→"BARREIROS TECIDO" 0.70).
- [x] No receipt appears in two different Matches rows. **MET
      2026-06-11** (3.8 bipartite; verified zero double-bound receipts
      across all three real months; same-currency false matches dropped
      4→1 / 0 / 5→1).
...
**Extended-calibration cross-check (2026-06-12) — all three travel
months, full line sets.** The travel ER PDFs (ER-00181 Copenhagen DKK,
ER-00183 Lisbon EUR, ER-00194 Rome EUR) were located + copied in;
extracted line-by-line (29 / 38 / 50 lines), totals matching each
report's per-currency summary TO THE CENT (DKK 4,005.08 + EUR 60.00;
EUR 4,156.74 + USD 45.13; EUR 3,224.26 + USD 145.54 — the DKK and
EU-format EUR parsing both work). Each month's full receipt set ran
against its Chase slice via `expense-recon calibrate`, all exit 0:

| Month | Charges | Receipts | FX multiplicity | Unmatched receipts | Invariant |
|---|---|---|---|---|---|
| Oct-24 Copenhagen (DKK) | 84 | 29 | 29/29 = 1.0x | 0 | OK |
