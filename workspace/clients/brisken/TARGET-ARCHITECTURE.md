...
- **Lovable hosting CAN serve real 301s.** The docs line "does not support 301
  ...
  a per-request CSP nonce, which no edge rule could produce). The 2026-09-09
  preference for Cloudflare is **superseded 2026-09-10**
  (OWNERSHIP-HANDOFF section 12): Cloudflare's CNAME setup needs the Business
  plan, so on any cheaper plan the whole zone's nameservers move, which puts
  Brisken's Microsoft 365 mail records in the blast radius for 18 redirects
  nobody has measured. The redirects go in app code as loader 301s, verified
  with `curl -I` before DNS moves; rome2026 is a GoDaddy subdomain forward.
...
**The SPA, same-origin. SUPERSEDED 2026-09-10** (OWNERSHIP-HANDOFF section
5): the SPA stays Lovable-hosted at `expenses.brisken.com`, because Lovable's
Publish is the only ship path a Brisken editor has, and co-hosting would
reintroduce a deploy nobody at Brisken can run. The paragraph stands as the
design if a developer is ever engaged to operate it. Built to a static Nitro
preset in a node build stage of the Dockerfile, served by recon at
`tools.brisken.com/` with a catch-all to `index.html`. `API_BASE` becomes the empty string; the CORS
...
**Reopened 2026-09-10:** with the SPA staying on Lovable hosting, a
parent-domain cookie would also travel to every Lovable-hosted and
Zoho-hosted `brisken.com` subdomain; keep the session cookie host-only and
redesign the sharing when P3 is actually scheduled (OWNERSHIP-HANDOFF step
15).
