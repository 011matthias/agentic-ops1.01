...
| Reply draft to Dirk | done | v6 staged 2026-09-07: short link mail pointing at the review packet; supersedes the long report versions. Nothing sent | User shapes + sends | none | `context/drafts/outreach-engine-plan-to-dirk.md` |
| September review packet (in-app sign-off) | done | SHIPPED + LIVE 2026-09-07 (PR #700, prod v13, browser-verified logged-in render): /review/september-2026 carries the 4 decisions as multiple choice + 3 wave sequences with editable verbatim wording + recipient lists (21/23/18). Responses stored per item; approve/edit/request-changes per wave. Review-only surface: no send path. Reseed via `lead-desk-maint seed-review` (packet JSON in gitignored context/lead-generation/) | Send Dirk the link mail; read his answers off the page | user sends the mail | PR #700; `web/review.py`; ARMING-DRILL.md step 7 unchanged |
