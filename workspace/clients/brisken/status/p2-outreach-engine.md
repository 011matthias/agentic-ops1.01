...
| Phase 1: arm the sender | in-progress | Dormant on prod (kill_switch=1 outside the one scoped ~30 s step-2 lift). Drill evidence (ARMING-DRILL.md): steps 1, 3, 4 PASS 2026-08-13; step 2 fresh-send leg PASS (draft-to-self tick claimed 1, draft verified in matthias Drafts, attempt leased un-acked; reply-threading leg runs between step 5's live send and the reply). Drill campaign live in status 'sending': sole contact admin@unpauseai.com (owner-chosen 2026-08-13), gates passed on owner delegation, default CC-to-Dirk cleared (would have leaked the drill), 24/7 fixture window, cap 2. Remaining: step 5 (needs Zoho test Lead + owner go + a reply from admin@), step 6 (BCC filing seen in CRM), step 7 (Dirk present) | Owner: create Zoho Lead for admin@unpauseai.com + give step-5 go; then steps 5+6 in one sitting | owner inputs; step 7 needs Dirk | ARMING-DRILL.md; rule_instantly_invasive |
