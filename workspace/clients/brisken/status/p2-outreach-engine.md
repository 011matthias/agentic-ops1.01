...
| September review packet (in-app sign-off) | done | SHIPPED + LIVE 2026-09-07 (PR #700, prod v13, browser-verified logged-in render): /review/september-2026 carries the 4 decisions as multiple choice + 3 wave sequences with editable verbatim wording + recipient lists (21/23/18). Responses stored per item; approve/edit/request-changes per wave. Review-only surface: no send path. Reseed via `lead-desk-maint seed-review` (packet JSON in gitignored context/lead-generation/). **Dirk opened it 2026-09-09T05:56:07Z** (prod `users.last_login_at`, the morning after the link mail) and has answered 0 of 8 items as of 2026-09-10. Readability pass shipped 2026-09-10 (PR #785, deployed, verified on prod): the mail bodies had inherited base.html's operator-editor monospace, so his four mails rendered as 12px code and each opened on a bare `{{first_name}}`; they now render as prose, auto-sized to full length, with one line per wave naming what the token becomes | Read his answers off the page; if still silent after the readability pass, a nudge in the Jul-25 thread | Dirk answer | PR #700 + #785; `web/review.py`; ARMING-DRILL.md step 7 unchanged |
