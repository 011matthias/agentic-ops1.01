...
| Phase 1: arm the sender | in-progress | Dormant on prod (kill_switch=1). Drill evidence (ARMING-DRILL.md): step 1 PASS; step 3 PASS 2026-08-13 (self-send 19:26:08Z, Sent Items imid readback + inbox landing verified); step 4 PASS (probe sent 19:26:47Z, Exchange NDR landed 19:26:54Z, bounce event + auto-suppress reason=bounced verified within 1 capture tick). Order corrected to 1 -> 3 -> 4 -> 2: step 2 requires the drill campaign, whose sole contact is the external test address (step-5 prep input). Setup script for the drill campaign staged | Get test address from owner; create drill campaign; run steps 2, 5, 6 | owner: test address + Zoho Lead; step 7 needs Dirk | ARMING-DRILL.md; rule_instantly_invasive |
