...
### 101. One click on 'Confirm all matched' or 'Reject N shown' commits up to 1,000 rows with no dialog, using a looser rule than the owner set (2026-09-17 audit draft #99, unranked; licence: defect, covered) (SHIPPED 2026-09-17, pending PR; SPA prompt `lovable-bulk-confirm-dialog-prompt.md` not applied)
...
### 131. The tool suggests pairs its own model calls 'likely NOT the same purchase' (2026-09-17 audit draft #129, unranked; licence: defect, covered) (SHIPPED 2026-09-17, pending PR; Shipped row 64)
...
**Shipped 2026-09-17 (pending PR).** `cli._apply_judgment` treats a model verdict at or below `fx_judgment_suggest_floor` as a rejection (was: below). A rejected pair whose own rate arithmetic sits in the clean band stays in review, `reason` = the tool's arithmetic, then "Kept for review although the model disagrees: " and the model's text; confidence stays the model's number. The band is item 81's, not a copy: the conversion, deviation and band moved into `matching.deterministic.reference_gap`, which `_fx_reference_fields` (the `fx.reference_*` block) and the judgment layer (`pair_reference_gap_band`, at the matcher's `_reference_rate_for`) both call. The three callers pass `cfg=match_cfg`; `tools/recon-match-attribution.py` passes it too when the module takes it, so the judge replays the rule. Measured on a DB copy pulled 2026-09-17 15:16 UTC (before = the branch base `99d2853e`, module identical to origin/main; after = this branch; `RECON_MODULE_SRC` proving the tree): class tables identical on July, August and the six bundles; exactly two July receipts change bucket, both labelled `excluded`: `0059` (NOBRE 65.23, 4.01%, model 0.20) review -> unmatched, and `0034` (Erste Fracht 21.00 EUR vs HOTEL AM TIERGARTEN 24.02, -1.59%, model 0.10, notes.csv "another merchant") unmatched -> review under the exception. That second move is the rule's cost, reported not netted: the exception keeps every clean-band pair the model rejects, whatever the number, and the vendor score does not separate `0034` (0.47) from the right pair `0062` (0.53). `0062` (NATHALIA, labelled right) stays in review with the reason reordered. August: nothing moves. Six bundles 70/95 unchanged (the bundle replay runs no judgment layer); pinned scorer SCORE 76.0, determ_ok 70/95, determ_wrong 0, nc_matched 0, before and after. Tests: `tests/test_rejected_fx_pair_item_131.py` (4, route-level through statement attach and `GET /api/runs/{id}`); `tests/test_fx_judgment_llm.py::test_apply_judgment_suggest_floor_keeps_stub_and_confident_pairs` now pins 0.21 kept, 0.20 unbound. regress_check red on all three wires: the `<=` comparison (2 of 4), the `cfg=` pass in `rematch_month` (2 of 4), the band exception (2 of 4). Suite 2196 passed / 2 skipped, six of them item 133's uncommitted characterization tests. Contract: api-contract "A pair the model rejects (item 131)". No SPA change.
...
**Characterization 2026-09-17 (NOT BUILT; no matcher change).** Pinned route-level against today's code in `tests/test_same_amount_other_merchant_item_133.py` (6 tests, not yet committed): an "Anthropic, PBC" USD 100.00 receipt, categorized by the merchant book so only the match is in question.

- BASE44 100.00 four days later (PROBABLE) or the same day (EXACT), alone: Reconciled, holds the receipt, vendor_pct 22; `requires_review` true on PROBABLE, false on EXACT; `turn: decide`, never self-confirms (`n_self_confirmed` 0), blocks `month_complete`, so `/publish` answers 400 `month_not_complete`. zoho.csv carries "Payment to BASE44" and reconciled.csv reads MATCHED with Receipt Vendor "Anthropic, PBC" while undecided. One bulk click books it: `POST .../decisions/confirm-matched` AND `.../confirm-ready` both confirm the vendor-22 pair (confirm-ready reads the CATEGORY verdict, not the match), after which the month is complete and publishes (200).
...
- The leak item 76/99/100/137 leave open: BASE44 the same day (EXACT, 0.99) outranks ANTHROPIC three days later (PROBABLE, 0.85) on confidence before the vendor is compared, holds the receipt, and the right charge lists NO candidate. Bulk confirm books the wrong pair; publish stays refused only because the right charge now needs a receipt, which a gray or yellow fill would close.
...
**Proposed narrowest rule (for an owner call, not built).** Two halves. (a) Item 101 is the real leak closer: both bulk routes select with `self_confirm_pairs`' match test (exact, unflagged, one candidate, vendor >= 75, not held / borrowed / rejected), so no click books a vendor-disagreeing pair; zero live pairs are affected beyond what a person confirms by hand. (b) Matcher, precedence only, in item 137's demotion shape: an exact-amount same-currency pair whose merchant disagrees yields to a rival for the SAME receipt whose merchant agrees (round B's dominance test: rival >= 0.5 and ahead by 0.25) by taking `requires_review` + a "the merchants differ" reason + confidence 0.55; with no such rival nothing changes, so Network Solutions, the Google twins and `0025` stay as they are and no pair on the eight datasets moves. Exports of undecided pairs stay by design unless Settings `export_approved_only` is turned on (owner setting, false live).
...
- **Cost centers and trips are untouched**: a company month whose cost centers resolve still sections per cost center, a trip per person. Nesting cards inside cost centers is the owner call this item left open.
...
|---|---|---|---|
| 64 | A pair the model rejects is not shown, unless the tool's own rate arithmetic is clean: a verdict at or below `fx_judgment_suggest_floor` (was: below) unbinds the pair; a rejected pair with `fx.reference_gap_band` "match" stays in review with the tool's reason first and "Kept for review although the model disagrees: ..." after it. One band function, `matching.deterministic.reference_gap`, feeds the FX block and the judgment layer | Item 131. The model answers exactly 0.20, so "below 0.20" let two rejected July pairs through with "likely NOT the same purchase" as their reason, one wrong (NOBRE 65.23, 4.01% off) and one right (NATHALIA 5.61, 1.5% off). Measured on a 2026-09-17 DB copy: class tables identical on July, August and the six bundles; July `0059` leaves review, `0034` (Erste Fracht, -1.59%, model 0.10, labelled excluded) returns to it under the exception, which is the rule's cost; scorer 76.0, 70/95, nc_matched 0 unchanged | 2026-09-17, pending PR; route-level `tests/test_rejected_fx_pair_item_131.py` (4), regress_check red on all three wires (floor comparison 2 of 4, `cfg=` in `rematch_month` 2 of 4, band exception 2 of 4); suite 2196 passed / 2 skipped, six of them item 133's uncommitted characterization tests |
