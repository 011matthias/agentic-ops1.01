...
|---|---|---|---|
| 95 | The month says when it was last matched: `last_rematch` + `rematch_pending` on both month payloads (pure reads of the snapshot's `rematch_log` / pending mark), and the Lovable prompt that renders them on the shared month header in EN + PT-BR (12 trigger labels) | Item 129 / owner notes #53 #54: a re-match ran silently; the only record was a counts-only operator event and a developer mail, and the SPA had nothing on the month payload to render | 2026-09-18, PR #1103; red-proven (five tests fail without the key); suite 2547 passed / 2 skipped; ruff clean; deploy + Lovable paste pending |
