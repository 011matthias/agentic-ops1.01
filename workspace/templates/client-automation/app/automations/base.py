...
class BaseAutomation(ABC):
    ...
    def run(self, trigger: str = "manual", dry_run: bool = False,
        ...
        """
        # Store test parameters for subclasses to use
        self._limit = limit
        self._record_ids = record_ids or []

        exec_logger = ExecutionLogger(self.automation_id, trigger=trigger)
        logger.info(f"Starting automation: {self.automation_id} (dry_run={dry_run}, limit={limit}, record_ids={len(record_ids) if record_ids else 0})")
