...
| `local-web-deploy.py [--no-deploy\|--skip-build\|--only SLUG]` | Canonical "ship local-web" path: build → `flyctl deploy` → fetch each `fly.dev` URL and assert the live origin serves THIS build (content-hash match on `/_astro/` refs). Cannot pass without the production URL serving the bytes you built. Kills the 2026-06-01 "declared live off a localhost preview, never deployed" class. `--no-deploy` re-verifies live==dist without redeploying. |
