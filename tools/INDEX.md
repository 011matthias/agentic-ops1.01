...
| `prompt-queue-ui.py [--port N] [--no-open]` | Localhost mini-UI for the skil_prompt-queue queue: view/add/edit/reorder/delete pending prompts + done.md tail. File stays source of truth (2s poll); mutations are hash-guarded AND content-addressed so the UI never acts on the wrong prompt after an agent drain. Embed in VS Code via Simple Browser. |
